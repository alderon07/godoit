...
func usage() {
  fmt.Printf(`godoit — minimal todo CLI
Version: %s (built: %s)
...
Run "godoit <command> -h" for detailed help on each command.
`, Version, BuildTime)
}
...
func main() {
  ...
  case "done":
    ...
    if doneFlags.NArg() < 1 {
      log.Fatal("Usage: godoit done <index>")
    }
  ...
  case "edit":
    ...
    if editFlags.NArg() < 1 {
      log.Fatal("Usage: godoit edit <index> [options]")
    }
  ...
  case "remove", "rm":
    ...
    if rmFlags.NArg() < 1 {
      log.Fatal("Usage: godoit rm <index>")
    }
